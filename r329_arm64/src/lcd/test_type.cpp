...
static int freetype_init(const char *font, int angle)
{
    ...
    /* 加载 face 对象 */
    error = FT_New_Face(library, "/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc", 0, &face);
    if (error)
    {
...
int main(int argc, char *argv[])
{
    ...
    /* 在 LCD 上显示中文 */
    int y = height * 0.25;
    lcd_draw_character(50, 100, L"hansolo hansolo", 0x000000);
    lcd_draw_character(50, y + 100, L"hansolo hansolo", 0x9900FF);
    lcd_draw_character(50, 2 * y + 100, L"hansolo hansolo", 0xFF0099);
    lcd_draw_character(50, 3 * y + 100, L"hansolo hansolo", 0x9932CC);
    /* 退出程序 */
    FT_Done_Face(face);
